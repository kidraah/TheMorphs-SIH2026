...
import { Layers, Database, Satellite, Radio, CheckCircle, RefreshCw } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded p-3.5 flex flex-wrap items-center justify-between gap-3 shadow-gov-sm">
        ...
        <button
          ...
          className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded flex items-center gap-1.5 border border-slate-300 shadow-sm transition-colors"
        >
          ...
          <tbody className="divide-y divide-slate-200">
            {telemetryFeeds.map((feed, idx) => (
              <tr key={idx} className="hover:bg-slate-50 transition-colors">
                <td className="p-2.5 font-bold text-[#0A3871]">{feed.name}</td>
