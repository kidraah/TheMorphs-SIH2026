...
export const RiskOutlookPage = () => {
  ...
  const { districtRiskData, fetchDashboardData } = useAppStore();

  // Make sure data is loaded if we navigate directly to this page
  useEffect(() => {
    if (!districtRiskData || districtRiskData.length === 0) {
      fetchDashboardData();
    }
  }, [fetchDashboardData, districtRiskData]);

  const fallbackData = districtRiskData || [];

  const filteredDistricts = fallbackData.filter((d) => {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white border border-[#E2E2E2] rounded p-3 shadow-gov-sm flex flex-wrap items-center justify-between gap-3 text-xs">
        ...
        <span className="text-slate-500 font-medium">
          Showing <strong>{filteredDistricts.length}</strong> of {fallbackData.length} Districts
        </span>
      </div>
            ...
            {filteredDistricts.length === 0 ? (
              <tr>
                <td colSpan={8} className="p-4 text-center text-slate-500">
                  {fallbackData.length === 0 ? "Loading data from backend..." : "No districts match current filters."}
                </td>
              </tr>
            ) : null}
