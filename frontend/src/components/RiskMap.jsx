    ...
    <div className="bg-white border border-[#E2E2E2] rounded shadow-sm overflow-hidden flex flex-col h-full">
      ...
      <div className={`relative w-full ${height} flex-1 overflow-hidden`}>
        ...
        >
          ...
          />
          {mapLayers['imdaa-risk'] && geoHeatmap && (
             <ImageOverlay 
                url={geoHeatmap.image}
                bounds={geoHeatmap.bounds}
                opacity={geoHeatmap.opacity}
                zIndex={400}
             />
          )}
          {mapLayers['qpe-rain'] && geoTrajectories && (
             <GeoJSON 
                key="trajectories"
                data={geoTrajectories}
                style={{ color: '#0EA5E9', weight: 2.5, dashArray: '4,4' }}
             />
          )}
